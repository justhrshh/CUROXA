...
router.post("/users", isHrOrAdmin, requireActiveSubscription, async (req, res) => {
  ...
  try {
    ...
    const newUser = await User.create({
      ...
      pan: req.body.pan,
      department: req.body.department || req.body.specialty || (role === 'doctor' ? 'General Medicine' : role === 'hr' ? 'Hospital Administration' : role === 'dpo' ? 'Data Protection & Compliance' : 'Administration'),
      designation: req.body.designation || (role === 'doctor' ? 'Consultant Practitioner' : role === 'hr' ? 'HR Manager' : role === 'dpo' ? 'DPO Manager' : role.charAt(0).toUpperCase() + role.slice(1)),
